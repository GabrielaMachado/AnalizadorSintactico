...
import edu.eam.analizadorsintactico.sentencias.implementaciones.Funcion;
 ...
 * @author dani0
 */
public class GramaticaFuncion implements Gramatica{
...
//Sentencia a retornar....
        ...
        if (lexema.getTipo() == TipoLexemaEnum.IDENT) {
            funcion.setIdent(lexema);
            posA++;
            lexema = arrayLexemas.get(posA);

            //nombre del atributo....
            if (lexema.getTipo() == TipoLexemaEnum.AGR_OPENP) {
                funcion.setOpenP(lexema);
                posA++;
                lexema = arrayLexemas.get(posA);

                if (lexema.getTipo() == TipoLexemaEnum.AGR_OPENP/*parametro*/) {
                    funcion.setParametro(lexema);
                    posA++;
                    lexema = arrayLexemas.get(posA);

                    if (lexema.getTipo() == TipoLexemaEnum.AGR_CLOSEP) {
                        funcion.setCloseP(lexema);
                        posA++;
                        lexema = arrayLexemas.get(posA);

                        if (lexema.getTipo() == TipoLexemaEnum.AGR_OPENKEY) {
                            funcion.setOpenKey(lexema);
                            posA++;
                            lexema = arrayLexemas.get(posA);

                            if (lexema.getTipo() == TipoLexemaEnum.ASIGNACION/*cuerpo*/) {
                                funcion.setCuerpo(lexema);
                                posA++;
                                lexema = arrayLexemas.get(posA);
